...
const services = [
  {
    ...
    icon: (
      <svg className="w-12 h-12 text-deep-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <section className="py-24 bg-gray-50 overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className={`${playfair.className} text-4xl md:text-5xl text-gray-900 mb-6`}>
            Our Services
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {services.map((service, index) => (
            ...
            >
              {/* Decorative accent */}
              <div className="absolute top-0 left-0 w-full h-1 bg-deep-green transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500"></div>
                ...
                <ul className="space-y-2 mb-8">
                  {service.features.map((feature, i) => (
                    <li key={i} className="flex items-center text-gray-600">
                      <svg className="w-4 h-4 mr-2 text-deep-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
